...
#ifndef DF_CONST_TIME_H
#define DF_CONST_TIME_H
 ...
 */
static inline u32 const_time_fill_msb(u32 val)
{
...
static inline u32 const_time_is_zero(u32 val)
...
static inline u32 const_time_eq(u32 a, u32 b)
{
    return const_time_is_zero(a ^ b);
}

/* Returns: -1 if a == b; 0 if a != b */
static inline u8 const_time_eq_u8(u32 a, u32 b)
{
 ...
 * length.
 */
static inline u32 const_time_eq_bin(const void *a, const void *b, size_t len)
{
    const u8 *aa = a;
    const u8 *bb = b;
    u8 res = 0;
...
/**
 * const_time_select - Constant time u32 selection
 ...
 */
static inline u32 const_time_select(
    u32 mask,
    u32 true_val,
    u32 false_val)
{
 ...
 * Returns: true_val if mask == -1, false_val if mask == 0
 */
static inline int const_time_select_int(u32 mask, int true_val, int false_val)
{
    return (int)const_time_select(mask, (u32)true_val, (u32)false_val);
}
...
{
    return (s8)const_time_select(mask, (u32)true_val, (u32)false_val);
}
 ...
 * is being copied.
 */
static inline void const_time_select_bin(
    u8 mask,
    const u8 *true_val,
    const u8 *false_val,
    size_t len,
...
static inline int const_time_memcmp(const void *a, const void *b, size_t len)
{
    ...
    {
        len--;
        int diff = (int)aa[len] - (int)bb[len];
        u32 mask = const_time_is_zero((u32)diff);
